  ...
  modal.innerHTML = `
    <div style="background: white; padding: 30px; border-radius: 8px; max-width: 500px; box-shadow: 0 4px 8px rgba(0,0,0,0.2);">
      ...
      <div style="background: #f5f5f5; padding: 15px; margin: 15px 0; border-radius: 5px;">
        ...
        ${(user.wallet || 0) < booking.totalBudget ? `
          <p style="color: red; font-weight: bold;">⚠️ Insufficient balance!</p>
        ` : `
      ...
      <div style="display: flex; gap: 10px;">
        ...
        ${(user.wallet || 0) >= booking.totalBudget ? `
          <button onclick="confirmPayment('${booking.id}'); this.parentElement.parentElement.parentElement.remove();" style="flex: 1; padding: 10px; background: #4caf50; color: white; border: none; cursor: pointer; border-radius: 4px; font-weight: bold;">Pay Now</button>
        ` : `
          <button onclick="window.location.href='payments.html';" style="flex: 1; padding: 10px; background: #ff9800; color: white; border: none; cursor: pointer; border-radius: 4px; font-weight: bold;">Add Funds</button>
        `}
      </div>
    </div>
  `;
...
function confirmPayment(bookingId) {
  ...
  // Reload bookings if on booking page
